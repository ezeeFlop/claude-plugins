#!/usr/bin/env python3
"""Spongram profile and macOS Keychain access. Never print credentials."""
import ctypes as c
import hashlib
import json
import os
...
DEFAULT_INSTANCE = "https://spongram.sponge-theory.dev"
...
def directory():
    return Path.home() / ".spongram" / "codex"
...
def validate_key(key):
    if not key or len(key) > 8192 or any(ord(ch) < 33 or ord(ch) > 126 for ch in key):
        raise SetupError("Enter the brain key without spaces or the Bearer prefix")
    return key
...
def load_profile():
    try:
        ...
        return {"instance_url": instance, "account": account(instance),
                "credential": profile.get("credential", {"kind": "spongram", "account": account(instance)})}
    except (OSError, ValueError, KeyError, TypeError):
        raise SetupError("Spongram is not configured. Run the spongram-setup skill.") from None
...
def read_secret(source):
    if source.get("kind") == "spongram":
        ...
        try:
            ...
            key = json.loads(raw).get("pluginSecrets", {}).get(source["plugin_id"], {}).get("brain_key")
