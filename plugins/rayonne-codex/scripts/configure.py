"""Configure the Rayonne plugin without exposing credentials to the agent."""
import argparse
import getpass
import json
import subprocess
...
def ask(prompt, default="", secret=False, terminal=False):
    ...
    script = '''on run argv
set answer to display dialog (item 1 of argv) with title "Rayonne — Codex" default answer (item 2 of argv) HIDDEN buttons {"Annuler", "Continuer"} default button "Continuer" cancel button "Annuler"
...
def probe(url, key):
    try:
        with httpx.Client(timeout=30, follow_redirects=False) as client:
            response = client.get(url + "/api/public/v1/overview", headers={"Authorization": "Bearer " + key})
        response.raise_for_status()
        data = response.json()
        if not isinstance(data, dict):
            raise ValueError()
    except (httpx.HTTPError, ValueError):
        raise SetupError("Workspace verification failed; check Rayonne URL and API key") from None


def save(url, key, store, source=None, verify=probe, read_only=False):
    url, key = normalize_instance(url), validate_key(key)
    verify(url, key)
    name = account(url)
    source = source or {"kind": "rayonne", "account": name}
    owned = source["kind"] == "rayonne"
    previous = store.read(name) if owned else None
    try:
        if owned:
            store.write(name, key)
        profile = {"instance_url": url, "account": name, "credential": source, "read_only": read_only}
...
def main():
    ...
    url = normalize_instance(ask("URL de Rayonne", default, terminal=args.terminal))
    candidate = None
    ...
    source = None
    if candidate and ask("Réutiliser la clé Rayonne existante du trousseau ? (oui/non)", "oui", terminal=args.terminal).lower() in ("oui", "o", "yes", "y"):
        source, key = candidate
    else:
        key = validate_key(ask("Clé API Rayonne (sans Bearer)", secret=True, terminal=args.terminal))
    read_only = ask("Mode lecture seule ? (oui/non)", "oui" if existing and existing.get("read_only") else "non", terminal=args.terminal).lower() in ("oui", "o", "yes", "y")
    save(url, key, Keychain(), source, read_only=read_only)
    print("Rayonne configured: workspace verified, key in Keychain. Start a new Codex thread.")
...
if __name__ == "__main__":
    try:
        ...
        sys.exit("Rayonne setup failed; no credential is logged.")
