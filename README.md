...
| **rayonne** | [Rayonne](https://rayonne.sponge-theory.dev) marketing autopilot for SaaS in Claude Code — product brief, Bullseye strategy, multi-channel content with its selling kit and media, native publishing or paste-ready Mode C kits, rendered videos and their storyboard workshop, AARRR metrics (81 tools, 8 commands including `/rayonne:rayonne-status`, `/rayonne:rayonne-content` and `/rayonne:rayonne-workspaces`). A key minted by an organisation admin drives every workspace of that organisation, and with the administration option it creates and deletes them. Requires the Business plan (API access). On enable, enter your `rk_…` API key (Réglages → Clés API, shown once). Needs [`uv`](https://docs.astral.sh/uv/) installed — the plugin runs its MCP server via `uvx rayonne-mcp`. |
